...
- **CLI**: `scan`, `update` (`--install-all` for ordered batch install), `install --device-id <id>`, `rollback --device-id <id>`, `details --device-id <id>`, `history` (audit trail of what was installed when)
