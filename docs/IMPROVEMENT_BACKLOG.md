...
| **「完成」の基準そのものは正しいか?**(自分が立てた受け入れ条件を疑う) | 基準を「作業が尽きた」から「**製品が自分について述べる主張のうち、機械的に強制されていないものが空か**」に変え、実際に対応表を作って空欄を探す | **空欄が2つあった。** README が最も強く謳う「**BYOVD 照合を全インストール/復元経路で**」と「**HTTPS-only downloads**」に強制が無かった。前者は CLAUDE.md に「経路を増やすときは必ず通すこと」と書いてあるだけで、実際に一度破られている(CAB バイパス)。`check-blocklist-paths.py` で両方を強制し、**新しい経路を足すと落ちる**ようにした。基準を弱めるのではなく強めた結果、残りの穴が見えた |
| **「ソースジェネレーターの実出力は検証できない」で終わってよいか?**(到達できない項目の周辺を疑う) | 生成器そのものは動かせないので、**再現が本体と同期しているか**を機械検証できないか試す | **否、検証できた。** `ui-typecheck` と `ui-run` の `Generated.cs` は生成結果を手で再現しているのに、**本体と同期しているかを何も見ていなかった**。本体に `[ObservableProperty]` を足して再現側に足し忘れると、ハーネスは本物と違う形のクラスを検証し続ける(XAML だけが参照するプロパティは、この環境では誰も参照しないのでコンパイルエラーにもならない)。`check-generator-contract.py` で命名規約に沿って照合する。**生成器の実出力そのものは依然として Windows 必須**だが、再現のズレという実際に起きうる失敗は塞げた |
...
| BC | **README の2大セキュリティ主張に強制が無かった**。「BYOVD 照合を全インストール/復元経路で」は CLAUDE.md に文章で書かれているだけで、実際に一度破られている(`.cab` はコンテナのハッシュしか見ておらず展開後の中身が無防備だった)。「HTTPS-only downloads」も同様に、判定を消しても何も検出しなかった | `tools/check-blocklist-paths.py` を新設。インストーラー/ドライバーストア書き込みを起動するメソッドを機械的に列挙し、**未登録の経路があれば落ちる**(新経路の登録忘れを捕まえる)。各経路が実際に照合を呼んでいることと、`DownloadUrl` で HTTP 要求を出すメソッドが `UriSchemeHttps` を確認していることも検査。4方向の変異で検出確認 |
| BB | **ソースジェネレーターの再現(`Generated.cs` 2ファイル)が本体と同期しているかを何も検証していなかった**。`[ObservableProperty]`/`[RelayCommand]` を本体に足して再現側に足し忘れても、この環境では誰も参照しないためコンパイルエラーにならず、ハーネスは**本物と違う形のクラス**を検証し続ける。逆に本体から消して再現側に残すと、ハーネスは通るのに Windows の実ビルドだけが壊れる | `tools/check-generator-contract.py` を新設。CommunityToolkit の命名規約(`_fooBar`→`FooBar`、`FooAsync`→`FooCommand`)で期待メンバーを導出し、両ファイルと過不足なく一致するか照合。`NotifyCanExecuteChangedFor` の対象実在も確認。3方向の変異で検出確認 |
